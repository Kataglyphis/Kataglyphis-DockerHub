...
- **Every BK chain ends with a MANDATORY smoke gate — do not route around it.**
  ...
  **On `-TargetArch arm64` the gate is NOT run, and that is not a skip — it is
  inapplicable (2026-08-23).** `smoke-test-container.ps1` verifies by EXECUTING
  the staged binaries (`ffmpeg -version`, `gst-inspect`, `LoadLibraryW` over
  every shipped DLL, a compile-and-run native probe, `import cv2`), and Windows
  x64 has no ARM64 emulation, so every one of those fails for the same
  uninformative reason whether the bundle is good or not. The driver reports
  `NOT APPLICABLE`, never "passed". **Do not lower the floors for arm64** to
  make it "run": a reduced `-SmokeMinPassed` would leave a number a later amd64
  change could quietly be measured against, which is exactly how this gate
  became decorative once before. The cross lane's verification is
  `verify-target-arch.ps1` (PE machine type over the whole install prefix,
...
The Windows lane source-builds the media stack with Ninja + clang-cl + lld-link (exceptions: CPython via `PCbuild\build.bat` with the VS ClangCL toolset; FFmpeg via MSYS2 `make` with `--toolchain=msvc`; GStreamer via Meson; LiteRT-**LM** via Bazel/bazelisk, `build-litert-lm-bazel.ps1`): CPython in the toolchain stage; ONNX Runtime → ONNX GenAI → OpenCV → FFmpeg in media-core; LiteRT (Ninja) → LiteRT-LM (Bazel) in media-litert; TVM → IREE in media-tvm; GStreamer in the merge stage. **That is the amd64 chain.** On `-TargetArch arm64` only `media-core` is buildable: `media-litert` and `media-tvm` are refused up front (`$crossBlockedBranches` in `build-buildkit.ps1`) because LiteRT-LM links a prebuilt **x86_64-only** static library and TVM builds its own LLVM with `X86;NVPTX` targets only — neither is fixable downstream. The merge's unconditional `COPY --from=media-litert`/`--from=media-tvm` are satisfied by the `media-branch-absent` stage, which stages those paths **empty** (a Dockerfile cannot branch on an ARG), so `$MergeRequiredBranches` is `media-core` alone on that lane. All version pins come from `linux/scripts/01-core/versions.env` — never restate versions here (the duplicated tables this section used to carry drifted, e.g. the GenAI/LiteRT-LM labels).
