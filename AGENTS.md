...
| BK cache huge but `buildctl du` shows `Reclaimable: 0B` | Refs pinned by named `bk-*` image generations + build history — prune cannot free anything | Remove obsolete generations from an ADMIN shell: `nerdctl --namespace buildkit rmi docker.io/local/kataglyphis:bk-<old>` then `buildctl prune --keep-storage 200gb`. The classic docker lane is separate (`docker rmi` works non-admin; verify a registry copy exists before deleting tagged finals). |
