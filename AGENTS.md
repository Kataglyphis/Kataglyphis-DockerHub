...
Two build lanes — **cross lane** (`linux/amd64` host, cross-compiles for all arches, tags `:cross-*`) and **runtime lane** (native target platform via QEMU, tags `:latest-cross-*`). Supported architectures: `amd64`, `arm64`, `riscv64`.

See `docs/linux-build-basics.md` for the image hierarchy diagram, `docs/overview.md` for the tag inventory, and the intro tables below (Dockerfiles, naming) for quick reference.
...
Tags generated by `linux/scripts/01-core/tag-naming.sh` functions. Cross lane: `cross_base_tag()`, `cross_compiler_tag()`, `cross_sdk_tag()`, `cross_media_tag()`, `cross_android_tag()`. Runtime lane: `runtime_base_tag()`, `runtime_package_tag()`, `runtime_wrapper_tag()`.
