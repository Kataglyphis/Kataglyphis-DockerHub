...
| Compile fully green, then `lld-link: error: undefined symbol` for template instantiations (`QkvToContext<...>`, `BiasSoftmaxImpl<double>`) at the DLL link — identical on every retry AND on a fresh cache mount | **the sccache nvcc path produced objects lacking arch/define-guarded instantiations** — runs 10+11 with launcher failed identically, runs 5+12 bare-nvcc linked green. Run 11's fresh mount falsified L0 poisoning; two candidate mechanisms remain: (H1) the decomposition silently drops guarded instantiations (its target-define drop is PROVEN at compile time — that is what patch 005 tolerates), or (H2) truncated crash-era objects served as hits from the WebDAV L2 (which was never reset). Same machinery also crashed the server on fused_moe (10054) and produced the `Severity::k0` phantom. Minimal wrapped-vs-bare nm-diff discriminator: scratchpad nvcc-repro (2026-08-10) | `SCCACHE_NO_CUDA_LAUNCHER=1` for the affected build (bare nvcc is the proven-green config). NEVER re-wrap CUDA on a new sccache without all THREE canaries: verify-cuda-cache.ps1 + fused_moe compile + a full providers_cuda LINK (the miscompile is invisible until link). C/CXX launcher stays safe. |
