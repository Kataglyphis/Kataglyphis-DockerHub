...
All stages use **Ninja+clang-cl+lld-link** (not MSBuild/VS generator). Two build lanes exist; **the BuildKit/containerd lane is the preferred one from 2026-08 on** (full CPUs + real layer caching), with docker-classic run+commit as the always-working fallback. For docker-classic operations use Stevedore's `docker.exe` (nerdctl needs an elevated shell for the containerd pipe).
