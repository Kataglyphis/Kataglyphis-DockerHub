...
| BK lane, disk is FINE: `failed to reimport snapshot: hcsshim::ExportLayer ... (0x3)` at finalize of a heavy media layer (GenAI/OpenCV class) — deterministic, every fresh snapshot, both finalize paths, survives host reboot | **Platform defect: lost HCS shutdown notification on heavy-churn container exit** (shutdown 30 s → terminate 30 s → `HCS_E_INVALID_STATE`, scratch never released). PARTIALLY Defender: the 2026-08-05 exclusions cured the hcs-temp FLAKE family and made TVM-class finalizes reliable (canary passed), but the first direct OpenCV finalize still 0x3'd with the original signature — the de-warming attempt was rolled back the same evening. Calm exits (ONNX ninja, cpython, LiteRT, torch) never trip it. | **SOLVED operationally via warm/materialize** (heavy builds in never-finalized warm solves, WebDAV tar handoff, calm materialize exports — docs/windows-builds.md § BuildKit lane). DO NOT retire the pattern without three consecutive clean OPENCV canaries (recipe: docs § roadmap; TVM is proven insufficient as specimen). 0x3 is deliberately EXCLUDED from the driver's transient-retry pattern — it must fail loudly. |
