...
| Compile fully green, then `lld-link: error: undefined symbol` for template instantiations (`QkvToContext<...>`, `BiasSoftmaxImpl<double>`) at the DLL link — identical on every retry AND on a fresh cache mount | **sccache's nvcc decomposition SILENTLY MISCOMPILES arch-guarded instantiation TUs** (device-conditional `#if __CUDA_ARCH__` blocks lost in its preprocessing; runs 10+11 with launcher failed identically, run 5 bare-nvcc linked green — the fresh-mount run 11 falsified the earlier "poisoned L0" attribution). Same root machinery also crashed the server on fused_moe (10054) and produced the `Severity::k0` phantom | `SCCACHE_NO_CUDA_LAUNCHER=1` for the affected build (bare nvcc is the proven-green config). NEVER re-wrap CUDA on a new sccache without all THREE canaries: verify-cuda-cache.ps1 + fused_moe compile + a full providers_cuda LINK (the miscompile is invisible until link). C/CXX launcher stays safe. |
