...
- **Process isolation for CPU parallelism.** `build.ps1` passes `--isolation process` on every `docker build`; the Hyper-V default is 2 CPUs, which pins every `ninja -j` to 2. `docker build` rejects `--cpu-count` and Windows ignores `--cpuset-cpus`, so isolation is the only lever. Regression symptom: `-j2` in `out\windows-build-logs\media-core.log`. Full rationale: `docs/windows-builds.md` § Build isolation and CPU parallelism.
...
- **Parallelism is memory-bounded, not CPU-bounded.** `Get-BuildJobCount = min(ProcessorCount, MEMORY_LIMIT_GB / MemGBPerJob)`. ONNX is tuned to ~4 GB/job (its CUDA/AVX-512 TUs are the RAM-heaviest; a few peak higher, but the `-j2` incremental retry absorbs the occasional OOM) → `-j12` at the default 48 GB, `-j14` at `-MediaMemoryGb 56`; OpenCV (~4 GB/job) and FFmpeg (~2 GB/job) run similar/higher. Since litert/tvm cache-hit, media-core effectively runs alone and can take most of host RAM — raise `-MediaMemoryGb` for more jobs. **You cannot reach `-j32` on ONNX**: 32 heavy TUs need ~128+ GB — RAM per job, not core count, is the ceiling.
