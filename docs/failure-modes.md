...
**Fix.** Order on any weird host: (1) `windows\scripts\diagnostics\probe-build-copy.ps1 -Heavy` is the ONLY valid verdict — only `-Heavy`-green counts; (2) RDNA4 dGPU present? elevated `toggle-rdna4-gpu.ps1 -Disable` → re-probe `-Heavy` → build → re-enable (the `Assert-NoActiveRdna4Gpu` preflight in `build-buildkit.ps1` enforces this); (3) after ANY red finalize: REBOOT before further A/Bs — a wedged host falsifies every experiment. Full A/B history, the falsification list and the superseded 2026-08-09 verdicts: `docs/windows-build-lanes.md` § RDNA4 dGPU layer-lock (A/B history and diagnostics).
