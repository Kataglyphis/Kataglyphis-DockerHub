...
| 4 | `windows/Dockerfile.media-merge-builder` (+ per-branch media builders) | `local/kataglyphis:windows-media` | AI/media stack: ONNX Runtime 1.27.0, ONNX GenAI 0.14.0, OpenCV 5.x, LiteRT 2.1.5, LiteRT-LM 0.13.1, TVM 0.25.0, FFmpeg `main` (`--enable-dnn --enable-libonnx`), GStreamer 1.29.2 — all source-built with Ninja/clang-cl in dependency order |
