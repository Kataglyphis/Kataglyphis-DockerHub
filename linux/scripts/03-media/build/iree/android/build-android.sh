...
if [ -f /opt/scripts/core/compiler-resolution.sh ]; then
  ...
  resolve_host_compiler() {
    case "$1" in
      c)   command -v gcc 2>/dev/null || command -v cc 2>/dev/null || command -v clang 2>/dev/null || true ;;
      cxx) command -v g++ 2>/dev/null || command -v c++ 2>/dev/null || command -v clang++ 2>/dev/null || true ;;
    esac
  }
fi
