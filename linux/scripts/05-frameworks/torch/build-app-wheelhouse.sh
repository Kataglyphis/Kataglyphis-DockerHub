...
# shell_quote_args lives in common.sh, which cross-env.sh does NOT source; without
# it both wheel builds die instantly, absorbed by the best-effort WARN path.
for _common in \
...
# versions.env, so PYTORCH_VERSION/TORCHVISION_VERSION hold even when this script
# runs outside the orchestrator (the usual case in the media app-wheelhouse stage).
for _lvf in \
...
for _evf in \
    ...
    if [ -f "${_evf}" ]; then
        # load_versions_env respects already-set env (orchestrator-forwarded pins
        # win); `set -a; source` would clobber them and invert that contract.
...
# IREE source tag for build_iree_wheels; mirrors versions.env IREE_VERSION, the
# fallback keeps this runnable outside the orchestrator.
...
if [ -f /opt/scripts/core/parallelism.sh ]; then
  ...
  # PyTorch's aten/autograd TUs peak ~4GB/cc1plus, so budget 4GB/job rather than
  # the generic 2GB (same OOM class as the litert build).
...
install_build_dependencies() {
    ...
    if command -v install_host_packages >/dev/null 2>&1; then
        # ccache is a HOST tool: it wraps the cross-compiler invocations that run on
        # the amd64 build host, so without it the multi-hour cross builds never cache-hit.
        ...
        # Target stdlib carries the _sysconfigdata__linux_<triplet>.py that
        # resolve_target_python_sysconfig_export needs for the TARGET Python ABI
        # metadata. Own apt call: install_target_packages groups are all-or-nothing.
        install_target_packages libpython3-stdlib || \
            warn "Target libpython3-stdlib unavailable; wheel builds will use host sysconfig"
        # Target libsleef-dev lets build_torch_wheel set USE_SYSTEM_SLEEF=1; bundled
        # sleef cross-builds its host-run codegen and dies "Exec format error".
...
install_native_build_dependencies() {
    # Best-effort host top-up; ccache is the lever (IREE's bundled LLVM is a ~1h
    # one-time compile against the persistent /var/cache/ccache mount).
...
prepare_build_environment() {
    # prepare_workspace is NOT called here - main() already ran it unconditionally,
    # so the wheelhouse dir exists even when this function bails out early.
    if cross_build_is_active; then
        ...
        # amd64 NATIVE: only IREE is source-built (torch/vision come from PyPI), so
        # no foreign target-Python staging and no cross toolchain.
        install_native_build_dependencies || {
    ...
    # Build-executor pins (supply-chain audit #18): these EXECUTE at build time.
    # setuptools keeps the <82 torch-compat ceiling; pure-python data deps stay floating.
...
# pytorch's setup.py does NOT forward CMAKE_ARGS -D flags to its own cmake; the
# CMAKE_TOOLCHAIN_FILE env var (cmake >=3.21) is the only channel that reaches it.
...
EOF
    # Also the only channel for cache vars setup.py won't forward: a HOST-runnable
    # protoc (see _torch_build_host_protoc).
    ...
    # Without these, FindPython reports "missing components: Development", pytorch
    # flips BUILD_PYTHON to OFF and the _C link dies "cannot find -ltorch_python".
    local py_inc py_lib
...
# Print the target sysconfigdata export (name + PYTHONPATH) only when the module is
# importable: the name without the module is an instant ModuleNotFoundError in setup.py.
...
append_common_cross_cmake_args() {
    ...
    # Explicit: an empty qemu_runner would leave the AND-list's 1 as the exit status
    # and set -e in the callers would turn that into a bogus build failure.
    return 0
}
...
# pytorch 2.13's cross bdist_wheel never packages the compiled torch/_C*.so (cmake
# links a generic torch/_C.so it does not copy), so `import torch` dies with the
# misleading "Failed to load PyTorch C extensions". Inject it and repack; no-op otherwise.
...
# These helpers read build_torch_wheel's locals via bash dynamic scoping - call
# them only from build_torch_wheel.
# shellcheck disable=SC2154

# Build a HOST protoc: bundled protobuf otherwise builds protoc for the TARGET and
# onnx codegen execs it on the host -> "Exec format error". Sets CROSS_HOST_PROTOC.
_torch_build_host_protoc() {
    ...
    # CMAKE_POLICY_VERSION_MINIMUM: bundled protobuf 3.13 asks for cmake <3.5, which
    # cmake >=4 refuses. Bare gcc/g++ already resolve host-side (/opt/cross-bin is triplet-only).
    if (cd "${src_dir}" && \
...
# Bundled sleef cross-builds its host-run codegen (mkrename) -> "Exec format error";
# prefer the target's system sleef when it landed. Sets use_system_sleef.
...
# Reads build_torch_wheel's locals + CROSS_HOST_PROTOC (via the generated toolchain
# file) through dynamic scope.
_torch_run_setup_py() {
    (
        cd "${src_dir}" && \
...
# Sets the TARGET_TORCH_WHEEL / TARGET_TORCH_VERSION globals.
_collect_torch_wheel() {
    local -a built_wheels=()
    # Must run BEFORE retag/collect so the repaired wheel flows the normal path.
...
build_torch_wheel() {
    ...
    # ccache launcher so the multi-hour aten compile cache-hits on rebuilds; guarded
    # so a host without ccache still builds.
...
# These helpers read build_torchvision_wheel's locals via dynamic scoping - call
# them only from build_torchvision_wheel.
# shellcheck disable=SC2154

# Reads build_torchvision_wheel's locals via dynamic scope.
...
# torch.utils.cpp_extension swallows ninja's output ("Error compiling objects for
# extension"); re-run ninja in the extension build dir to surface the real error.
...
# PyPI has no riscv64 build and only cp312-abi3 elsewhere, so every arch source-builds;
# the per-arch strategy (host/target stages, runtime-only cross) is owned by
# docs/linux-cross-builds.md, "IREE (Linux lane)". Not validatable on the amd64 dev host.
build_iree_wheels() {
    # Declared here so `set -u` does not trip on the native path, which leaves it
    # empty and lets the packaging step default to both projects.
    ...
    # ccache is the decisive lever: IREE pins its OWN LLVM fork, so it must compile
    # bundled LLVM/MLIR itself (~1h), and /var/cache/ccache is a persistent BuildKit
    # mount that makes that a one-time cost. See docs/build-cache-tiers.md.
    local -a ccache_cmake_args=()
    if command -v ccache >/dev/null 2>&1; then
        export CCACHE_DIR="${CCACHE_DIR:-/var/cache/ccache}"
        # Override UNCONDITIONALLY: Dockerfile.base already exports CCACHE_MAXSIZE=30G,
        # so a `:-` default never applied and the LLVM object sets evicted each other
        # (measured 2026-08-26, riscv64: 66% hit rate against the 30G limit).
        export CCACHE_MAXSIZE="${IREE_CCACHE_MAXSIZE:-64G}"
        export CCACHE_COMPRESS=1
        export CCACHE_SLOPPINESS="pch_defines,time_macros,include_file_mtime,include_file_ctime"
        mkdir -p "${CCACHE_DIR}" 2>/dev/null || true
        # The on-disk limit lives in the cache's own config: exporting the env var
        # alone leaves a 30G cache at 30G.
        ...
        # sccache takes its cap from SCCACHE_CACHE_SIZE, not from `ccache -M`, so the
        # override above must be mirrored there or IREE silently runs on the base 30G.
        _iree_launcher="$(compiler_cache_launcher 2>/dev/null || echo ccache)"
        case "${_iree_launcher}" in *sccache*)
            # Quiet by default; IREE_SCCACHE_LOG=sccache=info re-enables client logging.
    ...
    # Recursive submodules: torch-mlir + stablehlo give the compiler the full frontend
    # set (stablehlo + torch input dialects), not just TOSA/linalg.
    rm -rf "${src_dir}"
    ...
    # Force version-specific cp314 bindings, not IREE's default cp312-abi3: this sed
    # kills setup.py's hard-coded `_is_abi3_build` (no env escape hatch), the configure
    # half is -DIREE_ENABLE_PYTHON_STABLE_ABI=OFF. `False and ...` short-circuits.
    local _sp
    ...
    if cross_build_is_active; then
        ...
        # Stage 1 - NATIVE amd64 host build populating IREE_HOST_BIN_DIR. This function
        # runs inside the cross env, so CC/CXX/*FLAGS/CMAKE_TOOLCHAIN_FILE must be
        # stripped for both configure and build or the host tools come out foreign and
        # cannot drive the target build. The COMPILER=OFF-then-ON probe and what each
        # mode costs: docs/linux-cross-builds.md, "IREE (Linux lane)".
        ...
        # Tools the target stage needs from IREE_HOST_BIN_DIR. iree-tblgen is REQUIRED:
        # with COMPILER=ON the target builds its own FOR THE TARGET ARCH and runs it
        # during the build -> "Exec format error", [code=126] on VMOpEncoder.cpp.inc.
        # amd64 cannot see it - host and target are the same arch there.
        ...
        for host_compiler_mode in OFF ON; do
            ...
            # BuildKit collapses the ninja progress lines, so a bare failure surfaces
            # NO error - the log tail dump is the only way to see why the build died.
            if ! env -u CC -u CXX -u CPP -u CFLAGS -u CXXFLAGS -u CPPFLAGS -u LDFLAGS \
                     -u AR -u RANLIB -u CMAKE_TOOLCHAIN_FILE -u CMAKE_ARGS \
                ...
                # Deliberately NOT `continue`: ON's build graph is a strict superset
                # of OFF's, so it would fail the same way after a multi-hour LLVM compile.
        ...
        # BUILD_COMPILER stays OFF: the cross target is runtime-only, see
        # docs/linux-cross-builds.md, "IREE (Linux lane)". WERROR_FLAG=OFF: the cross
        # pyconfig.h redefines _POSIX_C_SOURCE against glibc's features.h and -Werror
        # makes that fatal. OUTPUT_FORMAT_C=OFF: it defaults ON under COMPILER=ON and
        # its EmitC codegen RUNS the target iree-compile at build time (code 127).
        ...
        # Bundled LLVM auto-detects LLVM_HOST_TRIPLE from the BUILD host, so a cross
        # iree-compile would default to embedded-elf-x86_64 and iree-run-module then
        # fails "HAL device not found". Pin both triples to the real target.
        ...
        # TARGET Python SOABI for the nanobind extensions: otherwise they are stamped
        # with the host's cpython-314-x86_64 suffix and the target cannot import
        # _runtime. Applied only here, after the host stage built with bindings OFF.
        ...
        # LLVM's CrossCompile.cmake defaults the NATIVE tblgen sub-build to the OUTER
        # (cross) compilers -> "Exec format error" on the build host. Pin it to the real
        # host toolchain. (';' is CMake's list separator - the value is ONE shell word.)
        ...
        rm -rf "${target_build}"
        # The cross target is runtime-only unless IREE_CROSS_BUILD_COMPILER=ON, so
        # demanding a compiler wheel afterwards would fail a correct build.
        ...
        # No host/target split, no toolchain file, no IREE_HOST_BIN_DIR, no qemu.
        # STABLE_ABI=OFF (+ the setup.py sed above) still forces cp314 wheels.
    ...
    # Which wheel projects the build tree exposes depends on the configure above:
    # native builds both, cross is runtime-only. Default to both for the native path.
...
main() {
    ...
    # torch/torchvision are cross-built ONLY for riscv64 (no upstream wheels) and are
    # best-effort - a failure must NOT short-circuit main() before the REQUIRED
    # build_iree_wheels (a ports.ubuntu.com outage once fail-open-skipped IREE).
    ...
    # IREE is REQUIRED on every arch: we ship version-specific cp314 wheels built from
    # source, never PyPI's cp312-abi3. A failed build must FAIL THE IMAGE; bypass only
    # with an explicit ALLOW_IREE_BUILD_FAIL=1 for a deliberate IREE-less image.
