...
# this copy had NO re-source guard, never tried the real 01-core/logging.sh,
# and defined no err() — a future `err` call would inherit whatever the
# caller happened to define, or explode.
[ -n "${_RUST_TOOLCHAIN_LIB_LOADED:-}" ] && return 0
_RUST_TOOLCHAIN_LIB_LOADED=1
_RUST_TOOLCHAIN_CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/../01-core"
if ! declare -F info >/dev/null 2>&1; then
  if [[ -f "${_RUST_TOOLCHAIN_CORE_DIR}/logging.sh" ]]; then
    # shellcheck source=../01-core/logging.sh
    source "${_RUST_TOOLCHAIN_CORE_DIR}/logging.sh"
  fi
fi
if ! declare -F info >/dev/null 2>&1; then
