...
# Ten slugs, one per shape: alpha (test), beta (own mutation), gamma (mutation on
...
# a sourced lib), theta (inline fn also stubbed in a lib nobody sources),
# script-tests (by construction).
_fixture() {
  local allow="$1" d
  d="$(mktemp -d)"
...
zeta"
python3 linux/scripts/verify_gamma.py --files ${_staged}
...
KNOWN_SLUGS=(alpha beta gamma omega delta \
             epsilon zeta eta theta script-tests)
run_check() { :; }
...
EOF
  printf '%s\nmutation-id:helper.y\n' "${allow}" > "${d}/linux/scripts/gate-proofs.allow"
  printf '%s' "${d}"
}
...
t_assert_contains "$(_run "${fix}")" "10 slugs; 8 proven; 2 unproven, 2 frozen" "the census line"
...
fix2="$(_fixture "$(printf 'delta\nomega')")"
_run "${fix2}" --write >/dev/null
t_assert_eq "0" "$(_rc "${fix2}")" "frozen by the fixture, so the tree is clean"
t_assert_contains "$(_run "${fix2}")" "1 mutation id(s) off-convention, 1 frozen" "and counted separately"
...
t_assert_contains "$(_run "${fix2}")" "prefix that names no preflight slug"
t_assert_contains "$(_run "${fix2}")" "mutation-id:helper.y"
printf 'delta\nomega\nmutation-id:helper.y\nmutation-id:ghost.z\n' \
...
fix="$(_fixture "$(printf 'delta\nomega')")"
sed -i 's/ theta script-tests)/ script-tests)/' "${fix}/linux/scripts/preflight.sh"
