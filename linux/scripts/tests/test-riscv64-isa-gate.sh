...
t_case "an object built without it FAILS -- this is the pre-RVA23 state"
t_assert_contains "$(_rvv_verdicts "RVARCH libopencv_core.so ${_OLD_OPENCV}")" \
  "BAD libopencv_core.so" "a sub-baseline object must not pass"
