...
def own_files(rel, text):
    return [rel] + imported_modules(rel, text)


def mutation_ids(entries, slug, files):
    """An entry is credited to the gate its id prefix names, and only when its target
    is that gate's own script or a module the script imports."""
...
def rows():
    ...
    for slug in order:
        ...
        row = {
            ...
            "tests": [t for t, txt in tests.items() if mentions(needle, txt)],
            "mutations": mutation_ids(entries, slug, files),
