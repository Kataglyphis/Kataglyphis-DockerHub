...
def scan_file(path, rel):
    ...
    # Arrays: NAME=( ... ) spanning lines, names may share a line.
    i = 0
    while i < len(raw):
        m = ARRAY_START_RE.match(raw[i])
        if m and ARRAY_NAME_RE.search(m.group(1)):
    ...
    # CPython dev-package table: row format "<dev-package> <required|optional> <ext>...".
    if rel.endswith("01-core/cpython-dev-packages.sh"):
        for n, text in enumerate(raw, 1):
            m = re.match(r'\s*"([a-z0-9][a-z0-9+.-]+)\s+(required|optional)\s', text)
            if m:
                reqs.append(Req(m.group(1), rel, n, "host+target", False, "cpython dev table"))

    # Call sites.
    guard_stack, refs = [], set()
    for lineno, acc in logical_lines(raw):
        ...
        # A for-loop over the array installs it one name at a time.
        if re.match(r"\s*for\s+\w+\s+in\s", stripped):
            for ref in re.finditer(r"\$\{([A-Za-z_][A-Za-z0-9_]*)\[[@*]\]\}", text):
        ...
        for m in CMD_RE.finditer(text):
            ...
            if base in TARGET_INSTALLERS or base in HOST_INSTALLERS \
               or base in APPEND_HELPERS or base.startswith("apt-get"):
                for ref in re.finditer(r"\$\{([A-Za-z_][A-Za-z0-9_]*)\[[@*]\]\}", args):
                    refs.add(ref.group(1))
            names = tokenize(args)
            if not names:
                continue
            guarded = (
                cmd in SELF_FILTERING
                or trailer.lstrip().startswith("||")
                or bool(re.match(r"\s*(if|elif|while|until|!)\s", pre))
                or "$(" in pre
                or block_guard
            )
            why = ""
            if guarded:
                why = ("self-filtering helper" if cmd in SELF_FILTERING
                       else "|| fallback" if trailer.lstrip().startswith("||")
                       else "apt probe in enclosing if" if block_guard
                       else "in a condition")
            scope = "target" if cmd in TARGET_INSTALLERS else "host"
            if cmd in APPEND_HELPERS:
                scope = "host"
            for name in names:
                reqs.append(Req(name, rel, lineno, scope, guarded, why))
    return reqs, arrays, refs
...
def main():
    ...
    def scope_arches(scope):
        return ARCHES if scope != "host" else ("amd64",)

    def refresh_installable():
        return {a: index[a][0] | set(index[a][1]) for a in ARCHES}

    installable = refresh_installable()
    suspects = {}
    for req in reqs:
        missing = [a for a in scope_arches(req.scope) if req.name not in installable[a]]
        if missing:
            suspects.setdefault(req.name, []).append((req, missing))

    # Second pass: -updates/-security, only for a name that would FAIL the gate.
    if any(not r.guarded and not is_vendor(r.path, r.name)
        ...
        for name in list(suspects):
            rows = [(r, [a for a in m if name not in installable[a]]) for r, m in suspects[name]]
