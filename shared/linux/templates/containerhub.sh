#!/usr/bin/env bash
# TEMPLATE — copy to <consumer>/scripts/linux/lib/containerhub.sh.
#
...
# build tooling that cannot be consumed from ContainerHub, because it is what
# *finds* ContainerHub.
#
...
# a lib/common.sh, containerhub_path/containerhub_source, a bare
# _CONTAINER_HUB_CORE variable, a _DRIVER variable inlined into every wrapper,
# CONTAINERHUB_SETUP_SCRIPT, and CONTAINERHUB_DIR/CONTAINERHUB_SCRIPTS_DIR — with
...
# Contract:
#
#   containerhub_path <relative>   -> absolute path, or fail naming it
#   containerhub_source <relative> -> source a library (load-guarded upstream)
#   containerhub_exec <relative>   -> exec a driver, passing "$@" through
#
# All three take paths relative to the ContainerHub root, e.g.
