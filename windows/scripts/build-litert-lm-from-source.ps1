...
# Both blocks are condition-gated on the breakage, so a complete export (0.13.1, or
# a future fixed tag) takes the upstream files untouched and the stubs self-retire.
...
# container already ships (LITERT_VERSION): graft support/, stage it via the same
# glob pipeline, and re-home the four relocated impl files the stale targets still
# reference by their old paths. Gated on the shim signature, so a future litert-lm
# whose CMake catches up takes upstream's layout untouched.
$mmShim = Join-Path $SourceDir 'runtime\util\memory_mapped_file.h'
if ((Test-Path $mmShim) -and
    ...
    # cmd shield: git writes progress to stderr (same pattern as the clones above)
...
# LEGACY (self-retiring): vcpkg protobuf was dropped from the base image 2026-08-03
# (setup-vcpkg.ps1 installs only zlib now), so on current bases this whole hide/
# restore block no-ops via its Test-Path gates. It stays for older base images.
...
if ($engineTxt -ne $engineOrig) {
    [System.IO.File]::WriteAllText($engineCmake, $engineTxt)
