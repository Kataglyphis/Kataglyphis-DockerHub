...
# resolved every one of these (Test-Toolchain.ps1 logs clang-cl,
# Install-Vs.ps1 logs the bootstrapper hash) — this writes them where they
...
# the value that actually floats (Install-Vs.ps1 pins only the MAJOR, via the
...
$manifest = [ordered]@{
    ...
    pinned = [ordered]@{
        ...
        # Build-OnnxGenaiFromSource.ps1 is written against.
