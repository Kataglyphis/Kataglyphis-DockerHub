...
$cmd.Add('@echo off')
$cmd.Add("rem Kataglyphis Windows media bundle -- $arch ($($info.PeMachineName)), written $stamp by write-bundle-manifest.ps1 (#130)")
...
$ps.Add("# Kataglyphis Windows media bundle -- $arch ($($info.PeMachineName)), written $stamp by write-bundle-manifest.ps1 (#130)")
...
$md.Add('')
$md.Add("Target: **$arch** (PE machine $($info.PeMachineName), clang target ``$($info.ClangTriple)``, wheel tag ``$($info.PythonWheelTag)``). Written $stamp by ``write-bundle-manifest.ps1`` in the merge stage; every path below was checked to exist at that moment.")
$md.Add('')
if ($cross) {
