...
#
#   # reference host
#   pwsh -File windows\scripts\compact-host-vhdx.ps1 -VhdxPath C:\cataglyphis-EXTREME.vhdx
#
#   # look first, change nothing (no downtime, no service stop)
#   pwsh -File windows\scripts\compact-host-vhdx.ps1 -VhdxPath D:\vm\build.vhdx -ReportOnly
#
...
if (-not $LogPath) { $LogPath = Join-Path $repoRoot 'out\compact-host-vhdx.log' }
...
foreach ($s in $Service) {
    try {
        ...
        $stopped.Add($s)
        Write-Step "$s stopped"
    } catch {
        Write-Step ("{0} STOP ERROR: {1}" -f $s, $_.Exception.Message) 'Yellow'
    }
}
