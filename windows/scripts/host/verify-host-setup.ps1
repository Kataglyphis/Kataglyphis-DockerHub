...
#
#   pwsh -File windows\scripts\host\verify-host-setup.ps1
#   pwsh -File windows\scripts\host\verify-host-setup.ps1 -SccacheEndpoint http://<ip>:5000
#
...
$shimFix = 'pwsh -File windows\scripts\host\deploy-shim-patch.ps1 -ShimPath <build>  (admin)'
if (-not (Test-Path $shim)) {
    ...
    if ($shimState -and $shimState.sha256 -and $shimState.shimPath -eq $shim) {
        ...
        Write-Check WARN ('runhcs shim looks PATCHED by SIZE only ({0:N0} bytes) - no recorded hash on this host' -f $size) `
            ...
            'pwsh -File windows\scripts\host\deploy-shim-patch.ps1 -ShimPath <build>  (admin) - writes the hash state file'
...
if ($cEnv -and ($cEnv -join ';') -match 'CONTAINERD_SHIM_RUNHCS_V1_TEARDOWN_TIMEOUT=') {
    ...
    Write-Check WARN 'CONTAINERD_SHIM_RUNHCS_V1_TEARDOWN_TIMEOUT not set on the containerd service' `
        ...
        'pwsh -File windows\scripts\host\apply-containerd-config.ps1  (admin)'
}
...
else { Write-Check WARN 'containerd debug logging OFF' 'the next snapshotter incident will carry no evidence' 'pwsh -File windows\scripts\host\apply-containerd-config.ps1  (admin)' }
...
if (Test-Path $buildctl) {
    ...
    else { Write-Check FAIL 'no gcpolicy on the worker - the store will grow until the disk dies' '' 'pwsh -File windows\scripts\host\apply-buildkitd-gcpolicy.ps1 -Force  (admin)' }
}
...
foreach ($driveLetter in (@('C', $repoDrive) | Select-Object -Unique)) {
    ...
    if ($freeGb -ge $MinFreeGb) {
        ...
        Write-Check FAIL "disk headroom ${driveLetter}: ${freeGb} GB is below ${MinFreeGb} GB (${role})" `
            ...
            'buildctl prune --free-storage <MB ABOVE total disk size - it is a minimum-free TARGET>; for a VHDX-backed volume: windows\scripts\host\compact-host-vhdx.ps1'
    }
}
...
if ($isAdmin) {
    ...
    else { Write-Check WARN ('Defender exclusions MISSING: ' + ($missing -join ', ')) 'load-bearing for the hcs-temp finalize flake family' 'pwsh -File windows\scripts\host\apply-containerd-config.ps1  (admin)' }
} else {
